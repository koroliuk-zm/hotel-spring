...
class UserControllerTest {
    ...
    void userPageShouldReturnUserPage() throws Exception {
        ...
        when(requestService.findUsersUnhandledRequests(anyLong())).thenReturn(requests);
        this.mockMvc.perform(get("/user"))
                .andExpect(status().isOk())
                .andExpect(view().name(Path.USER_PAGE))
                .andExpect(model().attribute("roomOrders", roomOrders))
                .andExpect(model().attribute("ordersHistory", ordersHistory))
                .andExpect(model().attribute("newOrders", requests))
    ...
    void freeRoomsPageShouldReturnRoomsPage() throws Exception {
        ...
        this.mockMvc.perform(get("/user/rooms"))
                .andExpect(status().isOk())
                .andExpect(view().name(Path.USER_ROOMS_PAGE))
    ...
    void roomOrderPageShouldReturnRoomOrderPage() throws Exception {
        ...
        this.mockMvc.perform(get("/user/rooms/order/{id}", 1L))
                .andExpect(status().isOk())
    ...
    void roomNewOrderPageShouldReturnRoomNewOrderPage() throws Exception {
        this.mockMvc.perform(get("/user/rooms/order/create_new_order/", 1L))
                .andExpect(status().isOk())
                .andExpect(view().name(Path.USER_CREATE_ORDER_PAGE))
    ...
    void saveNewRequestShouldRedirectToRoomsPage() throws Exception {
        ...
        this.mockMvc.perform(post("/user/rooms/order/create_new_order/"))
