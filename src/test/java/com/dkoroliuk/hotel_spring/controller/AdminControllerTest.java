...
class AdminControllerTest {
	...
	void adminPageShouldReturnAdminPage() throws Exception {
		...
		this.mockMvc.perform(get("/admin")).andExpect(status().isOk()).andExpect(view().name(Path.ADMIN_PAGE))
	...
	void getUserFormForAdminShouldReturnAdminUserForm() throws Exception {
		this.mockMvc.perform(get("/admin/users/new_user", 1L)).andExpect(status().isOk())
				.andExpect(view().name(Path.ADMIN_USER_FORM)).andExpect(model().attributeExists("userDTO"));
	}

	@Test
	void addNewUserShouldRedirectToAdminAllUsersPage() throws Exception {
		when(userService.saveUser(any())).thenReturn(new User());
		this.mockMvc.perform(post("/admin/users")).andExpect(status().is3xxRedirection())
	...
	void editUserFormShouldReturnUserEditPage() throws Exception {
		...
		this.mockMvc.perform(get("/admin/users/edit/{id}", 1L)).andExpect(status().isOk())
				.andExpect(view().name(Path.EDIT_USER_PAGE)).andExpect(model().attributeExists("userDTO"));
	}
	...
	void updateUserWhenUserValidShouldRedirectToAdminAllUsersPage() throws Exception {
		...
		this.mockMvc.perform(post("/admin/users/{id}", 1L, user)).andExpect(status().is3xxRedirection())
				.andExpect(view().name(Path.REDIRECT_ADMIN_ALL_USERS_PAGE));
	}
	...
	void getAllRoomsPageShouldReturnAllRoomsPage() throws Exception {
		...
		this.mockMvc.perform(get("/admin/rooms")).andExpect(status().isOk())
	...
	void getRoomFormForAdminShouldReturnAdminRoomForm() throws Exception {
		this.mockMvc.perform(get("/admin/rooms/new_room", 1L)).andExpect(status().isOk())
				.andExpect(view().name(Path.ADMIN_ROOM_FORM)).andExpect(model().attributeExists("roomDTO"));
	}

	@Test
	void addNewRoomShouldRedirectToAdminAllRoomsPage() throws Exception {
		when(roomService.saveRoom(any())).thenReturn(new Room());
		this.mockMvc.perform(post("/admin/rooms")).andExpect(status().is3xxRedirection())
	...
	void editRoomFormShouldReturnRoomEditPage() throws Exception {
		...
		this.mockMvc.perform(get("/admin/rooms/edit/{id}", 1L)).andExpect(status().isOk())
				.andExpect(view().name(Path.EDIT_ROOM_PAGE)).andExpect(model().attributeExists("roomDTO"));
	}
	...
	void updateRoomWhenRoomValidShouldRedirectToAdminAllRoomsPage() throws Exception {
		...
		when(errors.hasErrors()).thenReturn(false);
		this.mockMvc.perform(post("/admin/rooms/{id}", 1L, room)).andExpect(status().is3xxRedirection())
				.andExpect(view().name(Path.REDIRECT_ADMIN_ALL_ROOMS_PAGE));
	}
}
